...
export const FolderDetails: React.FC<FolderDetailsProps> = ({
  folder, onClearSelection, onDeleted, onEditClick, onUploadClick,
}) => {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const { folder: details, loading, error, fetchFolder } = useFolderDetails();
  const { deleteFolder, loading: deleting, error: deleteError } = useDeleteFolder();
    ...
    <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column', gap: 2, overflow: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <DynamicFolderIcon icon={f.icon} color={f.color} sx={{ fontSize: 36 }} />
          <Box>
            <Typography variant="h6" fontWeight={600} lineHeight={1.2}>{f.name}</Typography>
            <Typography variant="caption" color="text.secondary" fontFamily="monospace">{f.path}</Typography>
          </Box>
        </Box>
        <Box sx={{ display: 'flex' }}>
          {onEditClick && (
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
        <Box sx={{ p: 1.5, bgcolor: 'grey.100', borderRadius: 2 }}>
          <Typography variant="h6" fontWeight={600}>{f.documentCount}</Typography>
          <Typography variant="caption" color="text.secondary">Documents</Typography>
        </Box>
        <Box sx={{ p: 1.5, bgcolor: 'grey.100', borderRadius: 2 }}>
          <Typography variant="h6" fontWeight={600}>{details ? formatBytes(details.totalSize) : '—'}</Typography>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
        ...
        {details?.createdAt && (
          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Typography variant="caption">{timeAgo(details.createdAt)}</Typography>
          </Box>
        )}
        {details?.updatedAt && (
          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <Typography variant="caption" color="text.secondary">Modified</Typography>
            <Typography variant="caption">{timeAgo(details.updatedAt)}</Typography>
          </Box>
        )}
        ...
        <DialogContent>
          ...
          {f.documentCount > 0 && (
            <Alert severity="warning" sx={{ mt: 1.5 }}>
              This folder has {f.documentCount} document(s). Move or delete them first.
            </Alert>
          )}
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (ok) { setConfirmDelete(false); onDeleted(); onClearSelection(); }
            }}
