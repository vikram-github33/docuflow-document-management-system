...
import {
  ...
  TableSortLabel, IconButton, Tooltip, Avatar,
} from '@mui/material';
...
import { selectFolder, selectFile } from '../../redux/slices/folderSlice';
import { getFileIconConfig, formatFileSize, timeAgo, formatSizeBytes } from '../../utils/fileIcons';
...
export const FolderView: React.FC<FolderViewProps> = ({ folder }) => {
  ...
  return (
    <Box sx={{ height: '100%', overflow: 'auto', p: 2.5, display: 'flex', flexDirection: 'column', gap: 2 }}>
      {/* Folder header */}
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
        <Avatar sx={{ bgcolor: `${color}18`, width: 52, height: 52, borderRadius: 2 }}>
          <FolderIcon sx={{ color, fontSize: 30 }} />
        </Avatar>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="h6" fontWeight={600} sx={{ fontSize: 18, lineHeight: 1.2 }}>
            {folder.name}
          </Typography>
          <Typography variant="caption" color="text.secondary" fontFamily="monospace" sx={{ fontSize: 11 }}>
            {folder.path}
          </Typography>
          <Box sx={{ display: 'flex', gap: 0.75, mt: 0.5, flexWrap: 'wrap' }}>
            ...
            {folder.color && (
              <Box sx={{ width: 14, height: 14, borderRadius: '50%', bgcolor: color, mt: 0.2, border: '1.5px solid', borderColor: 'divider' }} />
            )}
          </Box>
        </Box>
      </Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1.5 }}>
        {[
          { label: 'Files',       value: folder.documentCount },
          ...
          <Paper key={label} variant="outlined" sx={{ p: 1.25, borderRadius: 2 }}>
            <Typography variant="h6" fontWeight={600} sx={{ fontSize: 20, lineHeight: 1 }}>
              {value}
            </Typography>
      ...
      {subfolderCount > 0 && (
        <Box>
          <Typography variant="caption" fontWeight={600} color="text.secondary"
            sx={{ textTransform: 'uppercase', letterSpacing: 0.8, fontSize: 10, mb: 1, display: 'block' }}>
              ...
              return (
                <Paper
                  ...
                  sx={{
                    ...
                    '&:hover': { borderColor: childColor, bgcolor: `${childColor}08`, transform: 'translateY(-1px)', boxShadow: 1 },
                  }}
                  onClick={() => dispatch(selectFolder(child))}
                >
                  <FolderIcon sx={{ color: childColor, fontSize: 22, flexShrink: 0 }} />
                  <Box sx={{ minWidth: 0 }}>
                    <Typography variant="body2" noWrap fontWeight={500} sx={{ fontSize: 12 }}>
                      {child.name}
                    </Typography>
                    <Typography variant="caption" color="text.secondary" sx={{ fontSize: 10 }}>
                      {child.documentCount} file{child.documentCount !== 1 ? 's' : ''}
                    </Typography>
                  </Box>
                </Paper>
              );
            ...
            <Table size="small">
              <TableHead>
                <TableRow sx={{ bgcolor: 'grey.50' }}>
                  ...
                  <TableCell sx={{ fontSize: 11, fontWeight: 600, py: 1, width: 80 }}>
                    <TableSortLabel
                  ...
                  <TableCell sx={{ fontSize: 11, fontWeight: 600, py: 1, width: 80 }}>Size</TableCell>
                  <TableCell sx={{ fontSize: 11, fontWeight: 600, py: 1, width: 100 }}>
                    <TableSortLabel
                ...
                {sortedDocs.map(doc => {
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={() => dispatch(selectFile({ doc, folderId: folder.id }))}
                    >
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <FileIcon fileType={doc.fileType} color={cfg.color} />
                          <Typography variant="body2" noWrap sx={{ fontSize: 12, maxWidth: 240 }}>
                            {doc.fileName}
                          </Typography>
                        </Box>
                      </TableCell>
      ...
      {subfolderCount === 0 && sortedDocs.length === 0 && (
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, color: 'text.secondary' }}>
          ...
          <Typography variant="caption">Upload files or create subfolders to get started</Typography>
        </Box>
      )}
    </Box>
  );
